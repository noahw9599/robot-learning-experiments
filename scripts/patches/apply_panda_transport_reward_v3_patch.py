...
MARKER = "# Robot Learning Experiments Panda transport_reward_v3"
BACKUP_SUFFIX = ".robot_learning_panda_transport_reward_v3_backup"
...
def apply(path: Path) -> None:
    ...
    text = replace_once(
        ...
        "robot_target_qpos=0.3,\n"
        "              # Panda transport_reward_v3: prioritize lift and transport.\n"
    ...
    text = replace_once(
        ...
        '        "prev_target_distance": jp.linalg.norm(target_pos - data.xpos[self._obj_body]),\n'
        '        "transport_progress": jp.array(0.0, dtype=float),\n'
        "    }\n",
        "reset-info anchor",
    )
    ...
    step_new = (
        "    # Panda transport_reward_v3: track lift, drag, and target progress.\n"
        ...
        "    )\n"
        "    target_distance = jp.linalg.norm(state.info[\"target_pos\"] - box_pos)\n"
        "    transport_progress = jp.clip(\n"
        "        (state.info[\"prev_target_distance\"] - target_distance) / 0.01,\n"
        "        -1.0,\n"
        "        1.0,\n"
        "    ) * state.info[\"lifted\"]\n"
        '    state.info["transport_progress"] = transport_progress\n'
        '    state.info["prev_target_distance"] = target_distance\n'
        + step_anchor
    )
    ...
    reward_new = reward_anchor + (
        '        # Panda transport_reward_v3: grasp, anti-drag, and transport.\n'
        ...
        '        "transport_progress": info["transport_progress"],\n'
    )
    ...
    metric_new = (
        ...
        '        transport_progress=state.info["transport_progress"],\n'
        "    )\n"
    )
